%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%  dcavity_compare_lossless.m
%
%  Cross-check that the boundary-integral (chunkie) solver and COMSOL give the
%  SAME physics for the LOSSLESS (PEC-wall) D-shaped cavity, at the level of
...
%     [S, U] = chunkie_solve(P, XY)   % dense BIE  (see dcavity_chunkie_Smatrix_lossless.m)
%     [S, U] = comsol_solve (P, XY)   % COMSOL FEM (see dcavity_comsol_Smatrix_lossless.m)
...
outdir  = fullfile(thisdir,'dcavity_compare_lossless_outputs');
...
P.minppw    = 20;
P.sigma_cond = Inf;                % LOSSLESS: PEC walls (Inf conductivity)
P.lossless   = true;
P.numch = numel(P.lead_ycen);
% chunkie surface admittance for the walls (= 0 here since lossless/PEC):
oph = P.c_light*P.omega;
P.Zs    = sqrt(1i*oph*P.mu0/(P.sigma_cond+1i*oph*P.eps0));   % = 0
P.gamma = 1i*oph*P.eps0*P.Zs;                                % = 0
% COMSOL LiveLink:
...
fprintf('=== D-cavity chunkie-vs-COMSOL compare (LOSSLESS PEC, %d ports) ===\n', P.numch);
...
plot_S_comparison(Sb, Sc, outdir, 'LOSSLESS (PEC) cavity');
